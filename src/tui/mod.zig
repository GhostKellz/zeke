...
pub const TuiApp = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, zeke_instance: ?*anyopaque) !Self {
        ...
        model_selector.setSelectedStyle(phantom.Style.default().withFg(phantom.Color.white).withBg(phantom.Color.bright_blue));
    ...
    fn streamAIResponse(self: *Self, input: []const u8) !void {
        ...
        const ai_response = try std.fmt.allocPrint(self.allocator, "🤖 ZEKE: I received your message: \"{s}\". This is now powered by phantom TUI v0.3.3 with zsync integration! The streaming text widget is working beautifully with true async support.", .{input});
